...
This hypothesis is at the basis of several techniques such as Bragg coherent diffraction imaging for which we use Fourier transforms to compute the scattered amplitude (sec. \ref{BCDI}).
...
\subsection{Refraction, reflection and absorption effects}\label{RefractionReflectionAbsorption}
...
The penetration depth of the x-rays in the material is detailed is sec. \ref{SXRD}.
...
\subsection{Lattice factor} \label{LatticeFactor}
...
The ability to effectively sample these fringes during experiments is at the root of the BCDI technique, to be able to perform the inverse Fourier transform of the Bragg peak to then retrieve the crystal shape (sec. \ref{BCDI}).
...
An example if given in sec. \ref{SIXS} for the SixS beamline of the synchrotron SOLEIL.
...
A detailed explanation of how to optimize BCDI experiments is given in sec. \ref{BCDI}.
