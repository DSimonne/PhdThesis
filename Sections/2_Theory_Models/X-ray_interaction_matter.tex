...
\begin{equation}
    F_{crystal}(\vec{q}) = \sum_j^{N_{atoms}} f_j(\vec{q}) e^{i\vec{q}.\vec{r}_j} \sum_k^{N_{uc}} e^{i\vec{q}.\vec{R_k}}
    \label{eq:Fcrystal}
\end{equation}
...
Where to mention that here we are using several hypothesis (Born approximation, DWBA, dipole), perfect crystals
}
