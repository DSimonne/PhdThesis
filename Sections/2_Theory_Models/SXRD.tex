...
A typical experiments yields structure factors measured across the scattering space reachable with the instrument.
...
If an atom was present in the center of the \numproduct{2x4} arrangement, \textit{i.e.} at (1, 2), the notation would change and become Pt(100)-c(2x4)-R\ang{0}.
...
This gives rise to a so called "rod" of intensity along the direction perpendicular to the truncated surface, a \textit{crystal truncation rod} (fig. \ref{fig:CTRConvolution}).
...
$q = q_z + l|\vec{c}^*|$, $l$ is the Miller index of the nearest Bragg peak.
...
The scattering vector $\vec{q}$ is determined by the incident and diffracted wavevectors,s and represents the difference between them (eq. \ref{eq:Q}).
...
This kind of measurement is called \textit{specular} measurement, for which the direction of the scattering vector is only perpendicular to the surface, yielding information about the material density only in the direction perpendicular to the surface.
